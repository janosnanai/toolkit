...
func TestTools_DownloadStaticFile(t *testing.T) {
	...
	testTools.DownloadStaticFile(rr, req, "./testdata", "babalili.jpg", "puppy.jpg")

	res := rr.Result()
	defer res.Body.Close()

	if res.Header["Content-Length"][0] != "355752" {
...
var jsonTests = []struct {
	...
	{name: "two json files", json: `{"foo": "bar"}{"hello":"bello"}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "empty body", json: ``, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "syntax error in json", json: `{"foo": bar"}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "disallowed unknown field in json", json: `{"foobar": "baz"}`, errorExpected: true, maxSize: 1024, allowUnknown: false},
	{name: "allowed unknown field in json", json: `{"foobar": "baz"}`, errorExpected: false, maxSize: 1024, allowUnknown: true},
	{name: "missing field name", json: `{jack: "baz"}`, errorExpected: true, maxSize: 1024, allowUnknown: true},
	{name: "file too large", json: `{"foo": "bar"}`, errorExpected: true, maxSize: 5, allowUnknown: true},
	{name: "not json", json: `Hello, I'm not a JSON - unfortunately`, errorExpected: true, maxSize: 1024, allowUnknown: true},
}
...
func TestTools_ReadJSON(t *testing.T) {
	...
	for _, e := range jsonTests {
		...
		// declare a variable to read to decoded json into
...
func TestTools_ErrorJSON(t *testing.T) {
	...
	if err != nil {
		t.Error("reciever error when decoding JSON", err)
	}
